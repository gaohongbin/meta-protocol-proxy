...
// 序列化 dubbo request
size_t DubboHessian2SerializerImpl::serializeRpcInvocation(Buffer::Instance& output_buffer, const MessageMetadata& metadata, Context& context) {
  // 在 Router::onMessageDecoded 中, 已经 getOriginMessage() 并且存入了 buffer 中。
  size_t origin_length = output_buffer.length();
  // 初始化一个 encoder, 这个 encoder 是 Hessian2::Encoder, 实现可以参考 https://github.com/alibaba/hessian2-codec/tree/main/hessian2
  // BufferWriter 是原生 envoy 里面定义的一个类,  class BufferWriter : public Hessian2::Writer
  Hessian2::Encoder encoder(std::make_unique<BufferWriter>(output_buffer));

  auto* invo = const_cast<RpcInvocationImpl*>(dynamic_cast<const RpcInvocationImpl*>(&metadata.invocationInfo()));
//  auto dubbo_version = invo->dubboVersion();
//  auto service_name = invo->serviceName();
//  auto service_version = invo->serviceVersion();
//  auto method_name = invo->methodName();

  // 写入 Variable Part
  bool result = false;
  result = encoder.encode(invo->dubboVersion());
  result |= encoder.encode(invo->serviceName());
  result |= encoder.encode(invo->serviceVersion().value());
  result |= encoder.encode(invo->methodName());

  // 写入 parameter Size
  result |= encoder.encode(context.parameterSize());

  auto& parameters = invo->parameters();
  for (auto& parameter : parameters) {
    result |= encoder.encode(*parameter);
  }

  result |= encoder.encode(invo->attachment().attachment());
...
DubboHessian2SerializerImpl::deserializeRpcInvocation(Buffer::Instance& buffer,
  ...
  invo->setParametersLazyCallback([delayed_decoder, context ]() -> RpcInvocationImpl::ParametersPtr {
    ...
    if (auto types = delayed_decoder->decode<std::string>(); types != nullptr && !types->empty()) {
      context->setParameterSize(*types);
...
size_t DubboHessian2SerializerImpl::serializeRpcResult(Buffer::Instance& output_buffer,
  ...
  // 在 Response 中 Variable Part 部分一共是两部分组成,
  // 第一部分: 一个 byte, 表示从服务器端返回的值类型. 这是 dubbo 定义的。
  // 第二部分: bytes, 返回的具体结果。
