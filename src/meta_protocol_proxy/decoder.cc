...
ProtocolState DecoderStateMachine::onDecodeStream(Buffer::Instance& buffer) {
  ...
  // 通过状态机进行解码
  // 这里解码完成以后, 会将 header 和 body 数据存入 metadata 中。
...
DecoderBase::DecoderBase(Codec& codec, MessageType messageType, std::shared_ptr<Envoy::TcloudMap::TcloudMap<std::string, std::string, Envoy::TcloudMap::LFUCachePolicy>> tcloud_map)
    : codec_(codec), messageType_(messageType), tcloud_map_(tcloud_map) {}
...
void DecoderBase::start() {
  // 这里第三个参数对应 Delegate, 传的是 DecoderBase 本身
  state_machine_ = std::make_unique<DecoderStateMachine>(codec_, messageType_, *this, getTcloudMap());
